...
// 1. IMPROVE CART EXTRACTION LOGIC GLOBALLY
...
<script id="sys-cart-fix">
        ...
        document.addEventListener('submit', (e) => {
            ...
            if (form && (form.action && form.action.includes('/cart/add') || form.querySelector('.product-form__add-to-cart') || form.querySelector('[name="add"]'))) {
                ...
                try {
                    ...
                    const titleEl = document.querySelector('.product-title') || document.querySelector('.product-form__title') || document.querySelector('h1') || document.querySelector('meta[property="og:title"]');
                    item.title = titleEl ? (titleEl.tagName === 'META' ? titleEl.content : titleEl.innerText.trim()) : (document.title.split('|')[0].trim());

                    // Variant info
                    const variantTitle = document.querySelector('.product-form__variant-title')?.innerText.trim();
                    if (variantTitle) item.title += ' - ' + variantTitle;

                    // Improved price extraction - LOOK FOR PRICE INSIDE FORM FIRST
                    let price = 12.00;
                    const priceSelectors = [
                        '.product-form .product-prices__price',
                        '.product-form .price__regular',
                        '.product-form [data-product-price]',
                    ...
                    // Fallback to window object price
                    ...
                    const imgEl = document.querySelector('.product-media__image') || document.querySelector('.product-media img') || document.querySelector('.product-form img') || document.querySelector('meta[property="og:image"]');
    ...
    if (html.includes('id="sys-cart-fix"')) {
        html = html.replace(/<script id="sys-cart-fix">[\s\S]*?<\/script>/, newFix);
    } else {
        html = html.replace('</body>', newFix + '</body>');
    }

    fs.writeFileSync(filePath, html);
});

console.log(`Updated cart extraction script in ${files.length} files.`);
